...
// Données intégrées: mapping des difficultés frontend -> enum Prisma
...
// Données intégrées: quizzes (extraits du frontend). Ici, on stocke seulement les métadonnées
// nécessaires au modèle Prisma. Les catégories seront mappées par nom -> id existant en base.
type PlayQuiz = {
  id: number;
  title: string;
  description: string;
...
type FrontQuestion = {
  ...
  question: string;
  options: string[];
  correctAnswer: number; // index dans options
  explanation?: string;
};

// Données intégrées (copiées du frontend) — 100% autonome
// PLAY_QUIZZES_DATA
const PLAY_QUIZZES_DATA: PlayQuiz[] = [
  // HISTOIRE (8 quiz)
  {
    id: 1,
  ...
  // GÉOGRAPHIE (extraits nécessaires)
  {
    id: 9,
  ...
  // ... Vous pouvez compléter avec le reste si nécessaire. Les IDs référencés dans QUIZ_QUESTIONS sont couverts.
];

// QUIZ_QUESTIONS
const QUIZ_QUESTIONS: { [quizId: number]: FrontQuestion[] } = {
  // QUIZ 1: Histoire de France - Révolution
  1: [
    {
      id: 1,
  ...
  // QUIZ 3: Seconde Guerre Mondiale
  3: [
    {
      id: 1,
  ...
  // QUIZ 8: Révolution Industrielle
  8: [
    {
      id: 1,
...
async function seedAll(): Promise<void> {
  ...
  // Données déjà intégrées ci-dessus

  // Récupérer toutes les catégories existantes pour map nom -> id
  ...
  // Choisir un auteur par défaut (le premier utilisateur)
  ...
  for (const q of PLAY_QUIZZES_DATA) {
    // Choisir la première catégorie disponible du quiz
    ...
    const difficulty = difficultyMap[q.difficulty] ?? Difficulty.MOYEN;

    // Upsert par titre + auteur + catégorie (approximatif pour éviter doublons)
    ...
    if (!existing) {
      ...
      // Nettoyer anciennes questions pour re-synchroniser
